...
export async function loadCapturedFixtures(): Promise<{
  ...
  // Prefer the captured (live skill output) over the hand-written bundled
  // fixture. Captured is regenerated by re-running find-abandoned-threads;
  // bundled is a single-thread placeholder.
  const abandonedCapturedMatches = import.meta.glob(
    '/src/fixtures/captured/abandoned-threads.json',
    { eager: true },
  ) as Record<string, { default: unknown }>;
  const abandonedBundledMatches = import.meta.glob(
    '/src/fixtures/abandoned-threads.json',
    { eager: true },
  ) as Record<string, { default: unknown }>;
  const abandonedMatches = Object.keys(abandonedCapturedMatches).length
    ? abandonedCapturedMatches
    : abandonedBundledMatches;
  ...
  if (Array.isArray(rawAbandoned)) {
    ...
    const source = Object.keys(abandonedCapturedMatches).length
      ? 'captured (live skill output)'
      : 'bundled (hand-written placeholder)';
    console.log(
      `[skillsClient] abandoned threads loaded: ${abandonedThreads.length} from ${source}`,
    );
  }
